...
/**
 * A file matcher abstraction that matches glob-patterns specified from a
 * provided file.
 *
 ...
 * </pre>
 * 
 * This will ignore the {@code .git} folder as well as all files ending with the
 * {@code .ignore} extension
 */
public class GlobFileMatcher {
	private final List<PathMatcher> matchers;

	/**
	 * Constructs a new {@code GlobFileMatcher} instance based on a root matching
	 * path and a file containing glob-style expressions
	 * 
	 * @param rootDirPath             the directory to match for globs in. This will
	 *                                be prepended to all glob loaded rules
	 * @param globIgnoreRulesFilePath path to a file containing glob rules
	 * @throws IOException if an OS error occurs when reading the provided glob
	 *                     ignore rules-file
	 */
	 ...
	 * 
	 * @param rootDirPath the directory to match for globs in. This will be
	 *                    prepended to the provided pattern
	...
	public void addGlobPattern(Path rootDirPath, String globPattern) {
		matchers.add(FileSystems.getDefault()
				.getPathMatcher("glob:" + rootDirPath.toString() + "/" + globPattern.trim()));
	}
	 ...
	 * @return {@code true} if any of the registered glob patterns matches the
	 *         provided filepath
	 */
