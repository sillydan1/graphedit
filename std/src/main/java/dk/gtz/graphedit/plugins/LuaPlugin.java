	...
	private void injectGlobalModule() {
		...
		geModule.put("api", new HashMap<>(Map.ofEntries(
				...
				Map.entry("keymap", new HashMap<>(Map.ofEntries(
						Map.entry("set", LuaUtils
								.wrap((key, fn, desc) -> DI.get(Keymap.class).set(
										LuaUtils.convert(key, LuaType.STRING),
										LuaUtils.convert(fn, LuaType.FUNCTION,
												e -> () -> e.call()),
										LuaUtils.convert(desc,
												LuaType.STRING)))),
						Map.entry("set_in_category", LuaUtils.wrap(
								(key, fn, desc, category) -> DI.get(Keymap.class).set(
										LuaUtils.convert(key, LuaType.STRING),
										LuaUtils.convert(fn, LuaType.FUNCTION,
												e -> () -> e.call()),
										LuaUtils.convert(desc, LuaType.STRING),
										LuaUtils.convert(category,
												LuaType.STRING))))))))));
