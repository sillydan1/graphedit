...
import dk.yalibs.yatarjan.Tarjan;
...
public class LTSLanguageServer implements ILanguageServer {
					...
					if (c.wasAdded()) {
						...
						changedVal.addListener((e, o, n) -> {
							...
							if (!syntaxName.get().equals(getLanguageName()))
								return;
							try {
								broadcastProgress(new ModelLanguageServerProgress("",
										ModelLanguageServerProgressType.PROGRESS,
										getServerName(), "calculating diffs"));
								var a = new ViewModelProjectResource(old.get(),
										ltsSyntax);
								if (!ViewModelDiff.areComparable(a, n))
									return;
								var diff = ViewModelDiff.compare(a, n);
								old.set(n.toModel());
								if (diff.getEdgeAdditions().isEmpty()
										&& diff.getEdgeDeletions().isEmpty())
									return;
								var cpy = ViewModelDiff.applyCopy(changedVal, diff);
								broadcastDiagnostics(c.getKey(),
										getSccs(c.getKey(), cpy));
							} finally {
								broadcastProgress(new ModelLanguageServerProgress("",
										ModelLanguageServerProgressType.END,
										getServerName(), "done"));
							}
						});
					}
	...
	private Collection<ModelLint> getSccs(String model, ViewModelProjectResource cpy) {
		var sccs = Tarjan.getStronglyConnectedComponents(getGraph(cpy.syntax())).stream()
				.filter(a -> a.size() > 1).toList();
