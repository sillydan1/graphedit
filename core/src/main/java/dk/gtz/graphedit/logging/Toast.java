...
/**
 * Graphedit toast utility. Use this to create notification "toasts" / popups
 * for the user.
 */
public final class Toast {
	...
	public static void warn(String msg) {
		if (DI.get(ViewModelEditorSettings.class).showWarnToasts().get())
			notify(msg, new FontIcon(BootstrapIcons.EXCLAMATION_TRIANGLE), Duration.seconds(7),
					Styles.WARNING);
	}
	...
	public static void error(String msg) {
		if (DI.get(ViewModelEditorSettings.class).showErrorToasts().get())
			notify(msg, new FontIcon(BootstrapIcons.EXCLAMATION_CIRCLE), Duration.INDEFINITE,
					Styles.DANGER);
	}
