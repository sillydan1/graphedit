...
public class ClipboardTool extends AbstractBaseTool {
	...
	public void copySelection(ViewportKeyEvent e) {
		try {
			...
			var selectedVertices = buffer.syntax().vertices().entrySet().stream()
					.filter(elem -> selectedElements.stream()
							.anyMatch(s -> s.id().equals(elem.getKey())))
			...
			for (var edge : selectedEdges.entrySet()) {
				if (!selectedVertices.containsKey(edge.getValue().source))
					selectedVertices.put(edge.getValue().source, buffer.syntax().vertices()
							.get(edge.getValue().source).toModel());
				if (!selectedVertices.containsKey(edge.getValue().target))
					selectedVertices.put(edge.getValue().target, buffer.syntax().vertices()
							.get(edge.getValue().target).toModel());
			}
			var modelGraph = new ModelGraph("", selectedVertices, selectedEdges); // NOTE: Declarations
												// string cannot be
												// copied by this tool
			var resource = new ViewModelProjectResource(buffer.metadata(),
					new ViewModelGraph(modelGraph, e.syntax())).toModel();
			var serializedModel = serializer.serialize(resource);
			var content = new ClipboardContent();
			content.putString(serializedModel);
			clipboard.setContent(content);
			logger.trace("copied {} vertices and {} edges to clipboard", selectedVertices.size(),
					selectedEdges.size());
		} catch (Exception exc) {
	...
	/**
	 * Paste the current content of the system clipboard. If the content is not a
	 * valid serialized model an error is logged and nothing will happen
	 * 
	 * @param e The key event
	 */
	public void pasteModel(ViewportKeyEvent e) {
		try {
			...
			var resource = new ViewModelProjectResource(serializer.deserializeProjectResource(content),
					e.syntax());
			var vertexOffset = new ViewModelPoint(e.editorSettings().gridSizeX().get(),
					e.editorSettings().gridSizeY().get());
			...
			for (var vertex : resource.syntax().vertices().entrySet()) {
				...
				rerolledVertex.put(rerolled,
						syntaxFactory.createVertexViewModel(rerolled, v.toModel()));
	...
	/**
	 * Copy the currently selected elements into the system's clipboard and delete
	 * the elements.
	 * 
	 * @param e The key event
	 */
