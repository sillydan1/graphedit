...
public class VertexDeleteTool extends AbstractBaseTool {
	 ...
	 * 
	 * @param buffer   The buffer that contains the vertex. This is used to push
	 *                 undo actions
	 ...
	 */
	public void delete(ViewModelProjectResource buffer, UUID vertexId, ViewModelVertex vertex,
			ViewModelGraph graph) {
		var linkedEdges = graph.edges()
				.entrySet()
				.stream()
				.filter(e -> e.getValue().source().get().equals(vertexId)
						|| e.getValue().target().get().equals(vertexId))
