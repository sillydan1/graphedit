 ...
 * When selected, click a vertex to start creating an edge and complete the edge
 * by clicking another vertex.
 * You can cancel edge creation by clicking at the canvas, the initial vertex
 * again or by pressing ESC
 *
 * Note that the action completes at edge completion (second click) rather than
 * edge creation (first click).
 */
public class EdgeCreateTool extends AbstractBaseTool {
	 ...
	 * 
	 * @return true when the tool is in a state where an edge is being created,
	 *         otherwise false
	 */
	 ...
	 * Will fail with a warning if the provided graph is not the same one you
	 * started creating the edge in
	 * 
	...
	public void release(ViewModelProjectResource buffer, UUID releaseTarget, ViewModelGraph graph) {
		if (!isReleaseTargetAndSourceTargetInSameGraph(currentEdge.get().source().get(), releaseTarget,
				graph)) {
	...
	public void create(UUID sourceTarget, ViewModelGraph graph, ISyntaxFactory factory) {
		...
		currentEdge = Optional.of(factory.createEdgeViewModel(currenEdgeId.get(),
				new ModelEdge(sourceTarget, tracker.getTrackerUUID())));
