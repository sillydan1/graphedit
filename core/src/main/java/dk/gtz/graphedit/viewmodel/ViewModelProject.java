 ...
 *
 * Not to be confused with {@link ViewModelProjectResource}, this class
 * represents a graphedit project.
 * 
 * @param metadata      A map of generic string-encoded metadata, useful for
 *                      syntax=specific data
 * @param name          The name of the project
 * @param rootDirectory The directory containing the on-disk file
 * @param isSavedInTemp Whether or not the project is currently a temporary
 *                      project
 * @param excludeFiles  A list of files to exclude from the project, can be
 *                      glob-specifications
 * @param runTargets    A list of {@link ViewModelRunTarget} associated with
 *                      this project
 */
public record ViewModelProject(
	...
	/**
	 * Constructs a new viewmodel project based on a model instance and an optional
	 * rootdirectory
	 * 
	 * @param modelProject  The model project datastructure to base on
	 * @param rootDirectory When provided, a valid file path containing the
	 *                      associated model project file, else will create a new
	...
	public ViewModelProject(ModelProject modelProject, Optional<String> rootDirectory) throws IOException {
		...
		for (var data : modelProject.metadata().entrySet())
			metadata.add(new Pair<>(new SimpleStringProperty(data.getKey()),
