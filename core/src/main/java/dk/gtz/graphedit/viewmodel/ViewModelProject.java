...
public record ViewModelProject(SimpleMapProperty<String,String> metadata, SimpleStringProperty name, SimpleStringProperty rootDirectory, SimpleListProperty<String> excludeFiles, SimpleListProperty<ViewModelRunTarget> runTargets) {
    public ViewModelProject(ModelProject modelProject, String rootDirectory) {
        this(new SimpleMapProperty<>(FXCollections.observableMap(new HashMap<>())), new SimpleStringProperty(modelProject.name()), new SimpleStringProperty(rootDirectory), new SimpleListProperty<>(), new SimpleListProperty<>(FXCollections.observableArrayList()));
