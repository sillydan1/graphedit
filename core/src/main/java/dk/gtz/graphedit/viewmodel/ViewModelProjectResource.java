 ...
 * Full file-on-disk model. Will include everything a graphedit project file
 * needs
 */
public class ViewModelProjectResource implements IFocusable, Property<ViewModelProjectResource> {
	...
	/**
	 * Constructs a new view model project resource based on the provided model
	 * project resource
	 * 
	...
	public ViewModelProjectResource(ModelProjectResource projectResource, ISyntaxFactory syntaxFactory) {
		this(new SimpleMapProperty<>(FXCollections.observableHashMap()),
				new ViewModelGraph(projectResource.syntax(), syntaxFactory));
	...
	/**
	 * Constructs a new view model project resource based on provided metadata and
	 * view model graph
	 * 
	 * @param metadata A map of metadata, useful for storing properties that aren't
	...
	/**
	 * Constructs a new model project resource instance based on the current view
	 * model values
	 * 
	 ...
	 * 
	 * @return optionally a string with the name of the syntax. empty if is not
	 *         present in the metadata field
	 */
