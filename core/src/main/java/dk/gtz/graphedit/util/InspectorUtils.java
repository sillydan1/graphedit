...
/**
 * General utilities for manipulating and creating {@link Property} inspectors /
 * editors.
 */
public class InspectorUtils {
	...
	/**
	 * General inspector creator function, use if you dont know what concrete type
	 * of {@link Observable} you have.
	 * 
	 * @param o the thing that the inspector modifies
	 * @return a javafx component that can modify the value of the provided
	 *         {@link Observable}.
	 * @throws RuntimeException   if the provided {@link Observable} is not
	 *                            supported
	 * @throws ClassCastException if the provided {@link Observable} is a collection
	 *                            type of non-observables or if it's a confusing
	 *                            ObjectProperty
	 */
	public static Node getObservableInspector(Observable o) {
		if (o instanceof BooleanProperty p)
			return getPropertyInspector(p);
		if (o instanceof DoubleProperty p)
			return getPropertyInspector(p);
		if (o instanceof FloatProperty p)
			return getPropertyInspector(p);
		if (o instanceof IntegerProperty p)
			return getPropertyInspector(p);
		if (o instanceof LongProperty p)
			return getPropertyInspector(p);
		if (o instanceof StringProperty p)
			return getPropertyInspector(p);
		if (o instanceof ObjectProperty p)
			return getPropertyInspector(p);
		// NOTE: Intentional classcast exception here. We cannot safely cast to generic
		// types because java is stupid
		if (o instanceof ListProperty p)
			return getPropertyInspector(p);
		if (o instanceof MapProperty p)
			return getPropertyInspector(p);
		if (o instanceof SetProperty p)
			return getPropertyInspector(p);
		throw new RuntimeException("No such property inspector implemented for type '%s'"
	 ...
	 * <br/>
	 * Note that this function contains intentional unsafe type casts and may throw
	...
	private static <T extends Enum<T>> ComboBox<T> createEnumComboBox(ObjectProperty<T> property) {
		var cmb = new ComboBox<T>(FXCollections
				.observableArrayList(property.get().getDeclaringClass().getEnumConstants()));
	 ...
	 *
	 * Note that this provides an {@link HBox} with the name of the property on the
	 * left.
	 * If you just want the map inspector, see
	 * {@link #createMapTextEditorNode(MapProperty)} instead.
	 * 
	 * @param property the thing that the inspector modifies
	 * @return a javafx component that can modify the value of the provided
	 *         {@link MapProperty}
	...
	private static void updateMapListView(VBox view,
			MapProperty<? extends Observable, ? extends Observable> property) {
	 ...
	 * Note that this provides an {@link HBox} with the name of the property on the
	 * left.
	 * If you just want the list inspector, see
	 * {@link #createListEditorNode(ListProperty)} instead.
	 * 
	 * @param property the thing that the inspector modifies
	 * @return a javafx component that can modify the value of the provided
	 *         {@link ListProperty}
	 ...
	 * Note that this provides an {@link HBox} with the name of the property on the
	 * left.
	 * If you just want the set inspector, see
	 * {@link #createSetEditorNode(SetProperty)} instead.
	 * 
	 * @param property the thing that the inspector modifies
	 * @return a javafx component that can modify the value of the provided
	 *         {@link SetProperty}
	...
	/**
	 * Get an inspector for a string property, but instead of a TextArea, it gives a
	 * TextField
	 * 
	 * @param property the thing that the inspector modifies
	 * @return a javafx component that can modify the value of the provided
