 ...
 * Note that there are differing "types" of the concept of "dragging" with a
 * mouse.
 * - If the intention is to have an object follow the mouse, this class simply
 * calls that "dragging"
 * - If the intention is to have a viewport and all its elements move with the
 * mouse, this class calls that "inverse dragging"
 * - If the intention is to have a drag and drop behavior, this class calls that
 * "dnd"
 */
public class DragUtil {
	...
	/**
	 * Add some event handlers to the provided node that makes it draggable through
	 * a {@link ViewModelPoint}
	 * 
	...
	public static void makeDraggable(Node node, ViewModelPoint point, Affine viewportAffine,
			ViewModelProjectResource buffer) {
		// NOTE: Must be properties due to java's final/effectively final lambda
		// restriction
		...
		node.addEventHandler(MouseEvent.MOUSE_RELEASED, event -> {
			if (undoAction.get() != null && redoAction.get() != null)
				buffer.getUndoSystem()
						.push(new Undoable("move action", undoAction.get(), redoAction.get()));
		});
	}

	/**
	 * Add event handlers to the provided node such that the provided {@link Affine}
	 * will be inversely translated on mouse dragging
