...
/**
 * An implementation of the {@link IObservableUndoSystem} interface that uses a
 * tree structure to store the undo history.
 */
public class ObservableTreeUndoSystem implements IObservableUndoSystem {
	...
	private ObservableUndoable getStringRepresentation(int indent, int index, Node node) {
		...
		return new ObservableUndoable("%s* %s\t(%s)".formatted(spaces, rep, node.action.getDescription()),
				node.action);
	}
