...
public class BindingsUtil {
	...
	/**
	 * Create a new {@link DoubleBinding} for the x-value on the edge of the
	 * provided shape and target position
	 * 
	 ...
	 * @throws RuntimeException if the provided {@code ViewModelVertexShape} value
	 *                          is not supported
	 */
	public static DoubleBinding createShapedXBinding(ViewModelPoint sourcePosition, ViewModelPoint targetPosition,
			ViewModelVertexShape shape) {
		return Bindings.createDoubleBinding(() -> {
			...
			throw new RuntimeException(
					"no binding implemented for shape %s".formatted(shape.shapeType().getName()));
		},
	...
	/**
	 * Create a new {@link DoubleBinding} for the x-value on the edge of the
	 * provided shape and target position in a rectangular manner
	 * 
	 * @param sourcePosition The source position
	 * @param targetPosition The target position. This will be the binding position
	 * @param shape          The shape dimensions to use
	 * @return The new binding
	 */
	public static DoubleBinding createRectangularXBinding(ViewModelPoint sourcePosition,
			ViewModelPoint targetPosition, ViewModelVertexShape shape) {
		return Bindings.createDoubleBinding(() -> {
			var diffX = targetPosition.getXProperty().get() - sourcePosition.getXProperty().get();
			var diffY = targetPosition.getYProperty().get() - sourcePosition.getYProperty().get();
			var intersection = ShapeUtil.rectangleIntersect(shape, diffX, diffY);
	...
	/**
	 * Create a new {@link DoubleBinding} for the x-value on the edge of the
	 * provided shape and target position in an oval manner
	 * 
	 * @param sourcePosition The source position
	 * @param targetPosition The target position. This will be the binding position
	 * @param shape          The shape dimensions to use
	 * @return The new binding
	 */
	public static DoubleBinding createOvalXBinding(ViewModelPoint sourcePosition, ViewModelPoint targetPosition,
			ViewModelVertexShape shape) {
		return Bindings.createDoubleBinding(() -> {
			var diffX = targetPosition.getXProperty().get() - sourcePosition.getXProperty().get();
			var diffY = targetPosition.getYProperty().get() - sourcePosition.getYProperty().get();
	...
	/**
	 * Create a new {@link DoubleBinding} for the y-value on the edge of the
	 * provided shape and target position
	 * 
	 ...
	 *                          is not supported
	 */
	public static DoubleBinding createShapedYBinding(ViewModelPoint sourcePosition, ViewModelPoint targetPosition,
			ViewModelVertexShape shape) {
		return Bindings.createDoubleBinding(() -> {
			if (shape.shapeType().get().equals(ViewModelShapeType.RECTANGLE)) {
				var diffX = targetPosition.getXProperty().get() - sourcePosition.getXProperty().get();
	 ...
	 * @return The new binding
	 */
	public static DoubleBinding createRectangularYBinding(ViewModelPoint sourcePosition,
			ViewModelPoint targetPosition, ViewModelVertexShape shape) {
		return Bindings.createDoubleBinding(() -> {
			var diffX = targetPosition.getXProperty().get() - sourcePosition.getXProperty().get();
			var diffY = targetPosition.getYProperty().get() - sourcePosition.getYProperty().get();
	 ...
	 * @return The new binding
	 */
	public static DoubleBinding createOvalYBinding(ViewModelPoint sourcePosition, ViewModelPoint targetPosition,
			ViewModelVertexShape shape) {
		return Bindings.createDoubleBinding(() -> {
			var diffX = targetPosition.getXProperty().get() - sourcePosition.getXProperty().get();
			var diffY = targetPosition.getYProperty().get() - sourcePosition.getYProperty().get();
	...
	/**
	 * Create a new {@link DoubleBinding} for a line that is bound to the end of a
	 * line
	 * This is useful for "sticking" something to the end of a line and have it
	 * follow the rotation
	 * 
	...
	/**
	 * Create a new {@link DoubleBinding} for binding the provided dependency to a
	 * scaled, rotated and translated offset (X-values)
	 * 
	 * @param dependency The double to offset
	 * @param offset     The offset to use
	 * @return A new binding that offsets dependency in a scaled and rotated manner
	 *         by the translation of the provided offset
	 */
	...
	/**
	 * Create a new {@link DoubleBinding} for binding the provided dependency to a
	 * scaled, rotated and translated offset (Y-values)
	 * 
	 * @param dependency The double to offset
	 * @param offset     The offset to use
	 * @return A new binding that offsets dependency in a scaled and rotated manner
	...
	/**
	 * Create a new {@link IntegerBinding} that maps a provided
	 * {@link StringProperty} using {@link Integer#valueOf}
	 * Note that the binding itself may throw a {@link NumberFormatException} if the
	 * string dependency has a non-int value
	 * 
	...
	/**
	 * Create a new {@link DoubleBinding} that maps a provided
	 * {@link StringProperty} using {@link Double#valueOf}
	 * Note that the binding itself may throw a {@link NumberFormatException} if the
	 * string dependency has a non-int value
	 * 
	 * @param dependency The string property to interpret
	...
	/**
	 * Create a new {@link StringBinding} that maps a T-typed {@link Property} to a
	 ...
	 * Create a new {@link StringBinding} that maps a T-typed {@link Property} to a
	 * string representation using the toString method.
	 * with a constant prefix and postfix prepended and appended respectively to the
	 * string value
	 * 
	 * @param <T>        The type of the wrapped value
	 * @param prefix     The prefix to prepend
	...
	public static <T> StringBinding createToStringBinding(String prefix, Property<T> dependency, String postfix) {
		return Bindings.createStringBinding(() -> prefix + dependency.getValue().toString() + postfix,
				dependency);
	}

	/**
	 * Create a new {@link DoubleBinding} that is bound to somewhere along the line
	 * by some scalar offset (X-values)
	 * 
	 * @param line   The line to offset along
	 * @param scalar The offset amount ranging from [0..1], lower or higher will
	 *               extend the point past the line
	 * @return The new binding
	 */
	...
	/**
	 * Create a new {@link DoubleBinding} that is bound to somewhere along the line
	 * by some scalar offset (Y-values)
	 * 
	 * @param line   The line to offset along
	 * @param scalar The offset amount ranging from [0..1], lower or higher will
	 *               extend the point past the line
