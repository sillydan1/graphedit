...
public class TipLoader {
	...
	public static TipContainer loadTips(String tipsFile) {
		try {
			...
			var tips = mapper.readValue(TipLoader.class.getClassLoader().getResourceAsStream(tipsFile),
					Tips.class);
			return new TipContainer(tips.tips());
		} catch (Exception e) {
			logger.error("Failed to load tips", e);
			return new TipContainer(List.of(new Tip("Error",
					"Failed to load tips file [font=monospace]'%s'[/font]".formatted(tipsFile))));
		}
	}
