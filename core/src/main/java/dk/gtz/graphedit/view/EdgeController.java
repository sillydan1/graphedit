 ...
 * If you extend from this and dont want all the features, you should overwrite
 * the unwanted initialize functions
 */
public class EdgeController extends Group {
	...
	/**
	 * A construct that tracks the mouse position. Useful when the edge is being
	 * edited
	 */
	 ...
	 * @param selectedTool   The object property specifying which tool is currently
	 *                       selected
	 * @param syntaxFactory  The associated syntax factory
	 * @param bufferKey      The key of the buffer
	 */
	public EdgeController(UUID edgeKey, ViewModelEdge edge, ViewModelGraph parentGraph, Affine viewportAffine,
	...
	private PointShape getPointShape(UUID lookupId) {
		if (lookupId.equals(tracker.getTrackerUUID()))
			return new PointShape(new ViewModelPoint(
					...
					BindingsUtil.createAffineOffsetYBinding(tracker.getYProperty(),
							viewportAffine)),
	...
	private Line initializeLinePresentation() {
		...
		edgePresentation.startXProperty().bind(
				BindingsUtil.createShapedXBinding(target.point(), source.point(), source.shape()));
		edgePresentation.startYProperty().bind(
				BindingsUtil.createShapedYBinding(target.point(), source.point(), source.shape()));
		edgePresentation.endXProperty().bind(
				BindingsUtil.createShapedXBinding(source.point(), target.point(), target.shape()));
		edgePresentation.endYProperty().bind(
				BindingsUtil.createShapedYBinding(source.point(), target.point(), target.shape()));
	...
	private void initializeEdgeEventHandlers(ObjectProperty<ITool> selectedTool,
			ViewModelEditorSettings editorSettings, String bufferKey) {
		addEventHandler(MouseEvent.ANY, e -> {
			var event = new EdgeMouseEvent(e, edgeKey, edgeValue, viewportAffine, syntaxFactory,
					parentGraph, bufferKey, editorSettings);
	...
	private void changeTargetBindPoints(UUID sourceId, UUID targetId) {
		...
		line.endYProperty().bind(
				BindingsUtil.createShapedYBinding(source.point(), target.point(), target.shape()));
		selectionHelperLine.startXProperty().bind(
				BindingsUtil.createShapedXBinding(target.point(), source.point(), source.shape()));
		selectionHelperLine.startYProperty().bind(
				BindingsUtil.createShapedYBinding(target.point(), source.point(), source.shape()));
		selectionHelperLine.endXProperty().bind(
				BindingsUtil.createShapedXBinding(source.point(), target.point(), target.shape()));
		selectionHelperLine.endYProperty().bind(
				BindingsUtil.createShapedYBinding(source.point(), target.point(), target.shape()));
	}
	...
	private void addCursorHoverEffect() {
		...
		addEventHandler(MouseEvent.MOUSE_EXITED,
				event -> lineArrowRight.getStyleClass().remove("stroke-hover"));
	}
}
