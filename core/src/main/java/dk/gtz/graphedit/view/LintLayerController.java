...
public class LintLayerController extends Pane {
	 ...
	 * @param viewportAffine The position, rotation and scale transform of the
	 *                       related viewport.
	 */
	...
	private ConvexHullPolygonController createConvexHullPolygon(Collection<ViewModelVertex> vertices,
			ViewModelLint lint) {
		var points = vertices.stream().flatMap(v -> {
			...
			return List.of(
					new ViewModelPoint(v.position().getXProperty().add(sizeX),
							v.position().getYProperty().add(sizeY)),
					new ViewModelPoint(v.position().getXProperty().subtract(sizeX),
							v.position().getYProperty().subtract(sizeY)),
					new ViewModelPoint(v.position().getXProperty().add(sizeX),
							v.position().getYProperty().subtract(sizeY)),
					new ViewModelPoint(v.position().getXProperty().subtract(sizeX),
							v.position().getYProperty().add(sizeY)))
		...
		switch (lint.severity().get()) {
			case ERROR:
				polygon.addStyleClass("stroke-error");
				break;
			case WARNING:
				polygon.addStyleClass("stroke-warning");
				break;
			case INFO:
				polygon.addStyleClass("stroke-info");
				break;
			default:
				break;
		}
