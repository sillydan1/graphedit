...
/**
 * View controller for the panel that contains tabs of model editor views as
 * tabs.
 */
public class EditorTabPaneController {
	...
	private void initTabpaneBufferContainer() {
		DI.get(IBufferContainer.class).getBuffers()
				.addListener((MapChangeListener<String, ViewModelProjectResource>) c -> {
					...
					if (c.wasAdded()) {
						...
						var editorController = new ModelEditorController(changedKey, changedVal,
								MetadataUtils.getSyntaxFactory(changedVal.metadata()));
						...
						var lastSavedModelSyntax = new SimpleObjectProperty<>(MetadataUtils
								.getSyntaxFactory(changedVal.getSyntaxName().get()));
						changedVal.addView(tab);
						changedVal.addListener((e, o, n) -> {
							var a = new ViewModelProjectResource(lastSavedModel.get(),
									lastSavedModelSyntax.get());
						...
						EditorActions.addSaveListener(() -> {
							...
							lastSavedModelSyntax.set(MetadataUtils.getSyntaxFactory(
									changedVal.getSyntaxName().get()));
							tab.unsetHighlight();
						});
					...
					if (c.wasRemoved())
						c.getValueRemoved().getViews()
								.forEach(v -> tabpane.getTabs().remove((Object) v));
				});
	}
	...
	private void initTabEventHandlers() {
		Platform.runLater(() -> {
			tabpane.getScene().addEventHandler(KeyEvent.ANY, e -> {
				var c = tabControllerMapping
						.get(tabpane.selectionModelProperty().get().getSelectedItem());
				if (c != null)
					c.onKeyEvent(e);
			});
		});
