...
public class RunTargetsEditorController {
	...
	private void initAddButton() {
		...
		addButton.setOnAction(e -> project.runTargets().add(new ViewModelRunTarget("default", "ls", ".", true,
				true, false, new ArrayList<String>(), new HashMap<String, String>())));
	}
	...
	private void initRemoveButton() {
		...
		removeButton.setOnAction(e -> {
			...
			if (!didRemove)
				logger.warn("Could not remove '%s' RunTarget from list"
						.formatted(selectedRunTarget.get().name()));
			selectFirstElement();
		});
	}
	...
	private void renderRunTargetInspector(ViewModelRunTarget runTarget) {
		...
		var runBeforeTile = new Tile("Save before run",
				"Whether to save the model before running this command");
		...
		var restartAfterTile = new Tile("Restart after run",
				"Whether to restart the editor after running this command (successfully)");
		...
		addButton.setOnAction(e -> runTarget.environment().add(new ViewModelEnvironmentVariable(
				new SimpleStringProperty("KEY"), new SimpleStringProperty("VALUE"))));
