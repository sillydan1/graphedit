...
public class ConvexHullPolygonController extends Group {
	...
	private void updateOnChangedEvent(ObservableValue<? extends ViewModelPoint> e, ViewModelPoint o,
			ViewModelPoint n) {
		update();
	}
	...
	private void findHull(List<ViewModelPoint> sk, ViewModelPoint p, ViewModelPoint q, List<ViewModelPoint> hull) {
		...
		for (var point : sk) {
			var height = -getLineDistance(p, q, point); // NOTE: Inverted, because y-axis is inverted in
									// javafx
			if (height < maxHeight)
				continue;
			maxHeight = height;
