 ...
 *
 * Language servers provide the language smarts that help developers create
 * better, safer and valid models.
 * A server provides diagnostics, lints, optimizations, suggestions and
 * notifications.
 */
public interface ILanguageServer {
	...
	/**
	 * Get the name of the language server (not to be confused with
	 * {@link getLanguageName})
	 * 
	 ...
	 * 
	 * @param projectFile     The Graphedit project file. Note that this can also
	 *                        just be a temporary directory
	 * @param bufferContainer The collection of buffers, likely (but not guaranteed)
	 ...
	 * This will be called from a seperate thread and will be interrupted via
	 * {@link Thread#interrupt} when it's time to close.
	...
	/**
	 * Add a callback function to call when language server progress notification
	 * happens.
	 * 
	 * @param callback The function to call when a progress notification should be
	 *                 shown.
	 */
